            ...
            while(escape != 1 && playerStun < 1){
                ...
                if(choice == 1){//atacar
                    ...
                    if((roll + ((player->weapon) ? player->weapon->attackMod : 0) + playerTempStatus.attackMod) >= (enemy->armorClass+enemyTempStatus.defenseMod)){
                        printf("Você acerta seu golpe desferindo %d de dano ao adversário!\n", ((player->weapon) ? player->weapon->damage : 1));
                    ...
                    int result = useItem(player, enemy, &enemyTempStatus, &enemyStun);
                    if(result){
                        break;
                    }
                    system(CLEAR_SCREEN);
...
void gamePlayLoop(Player *player, Room *rootRoom){
    ...
    charPrint("E ao lado da espada alguns tickets de feriado\n");
    grabItem(player, startTickets);

    //cheat
    // Item *cesar = loadItem("hack");
    // grabItem(player, cesar);
    // Item *pinguim = loadItem("roupa de pinguim");
    // grabItem(player, pinguim);
